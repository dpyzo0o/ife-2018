...
function init() {
  ...
  if (location.hash.length > 1) {
    let selected = JSON.parse(decodeURI(location.hash.slice(1)));

    selected.product.forEach(el => {
      document.getElementsByName(el)[0].checked = true;
    });

    selected.region.forEach(el => {
      document.getElementsByName(el)[0].checked = true;
    });

    document.getElementById('regionAll').checked = selected.region.length === 3;
    document.getElementById('productAll').checked = selected.product.length === 3;
  } else if (localStorage.getItem('selectedData')) {
